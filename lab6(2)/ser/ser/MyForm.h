...
#pragma endregion
	...
	private: System::Void btn_start_Click(System::Object^ sender, System::EventArgs^ e)
	{
		...
		HANDLE pipe = CreateNamedPipe(L"\\\\.\\pipe\\mypipe", 
			...
			PIPE_UNLIMITED_INSTANCES, 1, 1, 0, NULL); //создаём канал;
		...
		while (input != "Finished!")
		{
			...
			else if (input == "Clear")  //для очищения формулы;
			{
		...
		file << "Log ended!";
		file.close();
		DisconnectNamedPipe(pipe);  //отключаемся от канала;
